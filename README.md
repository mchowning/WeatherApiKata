...
This test is going to be very similar to the test that we already did to test that the WAKWeatherService executed the success block it received in its getCurrentTemp: method after a successful network call.  The only difference is that this time we're going to create a fake responseObject to pass as the second parameter when we run the "blockThatWouldBeExecutedOnSuccessfulNetworkCall".  Then, instead of just testing the the original block was executed, we're going to actually test that the original block received the correct temperature as a parameter.  You could say that this second test renders the first test unnecessary because if it passes, the first test necessarily would have passed.  As a result, once you have this second test passing, feel free to refactor by removing the first test if you would like.
